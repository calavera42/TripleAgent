...
void AnimationProvider::SetAnimationState(AnimationState as)
{
	LastState = AnimState;
	AnimState = as;
}
...
void AnimationProvider::Update()
{
	...
	{
		bool lastFrame = Frame == CurrentAnimation.Frames.size() - 1;
		bool nullFrame = currentFrame->FrameDuration == 0;
		...
		Interval = GetCurrentFrame()->FrameDuration;
		AdvanceFrame(currentFrame->Branches);

		if (GetCurrentFrame()->FrameDuration == 0)
			Update();
...
short AnimationProvider::GetAnimationLength()
{
	return CurrentAnimation.Frames.size();
}
...
void AnimationProvider::LoadAnimationFromState(AgentState state)
{
	...
	StateInfo* stateInfo = AgFile->GetStateInfo(stateLookup[(int)state]);
